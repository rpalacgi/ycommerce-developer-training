 ...
 *
 * The following class supports a Tesd-Driven-Development (TDD) approach for the
 * SAP Hybris Commerce Developer training, part 2.
 *
 */
@IntegrationTest
public class DefaultBookFacadeTest extends ServicelayerTransactionalTest {
	 ...
	 *
	 * trendingBooks - a List<String> containing the ISBN10 values of the top 5
	 * rented books
	 *
	 */
	@Test
	public void testGetTrendingBooks() {
		...
		final List<BookData> FiveTrendingBooks = bookFacade.getTrendingBooks(5);

		// 1.1 Fail if it doesn't return specified number of books (5)
		if (FiveTrendingBooks.size() != 5) {
			fail("The getMostRentedBooks() method did not return the requested number of books");
		}

		// 1.2 Fail if the ISBN10 numbers of the returned list don't match the
		// numbers in trendingBooks.
		final List<String> trendingISBN = new ArrayList<String>();
		for (final BookData bookData : FiveTrendingBooks) {
			trendingISBN.add(bookData.getISBN10());
		}

		if (!trendingISBN.containsAll(trendingBooks)) {
	 ...
	 *
	 *
	 * We will create multiple rentals for some books, making it possible to
	 * determine our five "most rented" books. We store the five most-rented
	 * books in the following list:
	 *
	 * trendingBooks - a List<String> containing ISBN10 values
	 *
	 *
	 * Books 1 & 2 have 3 rentals each Books 3, 4 & 8 have 2 rentals each Books
	 * 5, 6 & 7 have 1 rental each
	 *
	...
	public void initialize() {
		...
		// Find the bookstoreProductCatalog:Staged catalog version to insert
		// books into
		final CatalogVersionModel stagingCatalog = catalogVersionService.getCatalogVersion("bookstoreProductCatalog",
				"Staged");
		...
		// (remember: since we extend ServicelayerTransactionalTest, our changes
		...
		/*
		 * // BOOK 7 -- 1 rental book = modelService.create(BookModel.class);
		 * book.setCode("Red Bird: Poems");
		 * book.setCatalogVersion(stagingCatalog);
		 * book.setApprovalStatus(ArticleApprovalStatus.APPROVED);
		 * book.setLanguage("en"); book.setPublisher("Beacon Press");
		 * book.setISBN10("9780807068922"); book.setISBN13("9780807068922");
		 * book.setRentable(true); books.add(book);
		 *
	 ...
	 * createRental
	 *
	 * Utility method to create a rental for the given book, using the provided
	 * rentalId.
	 *
	 * @param book
	 * @param rentalId
